...
- [ ] 1. Local Diff Planning — Change Source Abstraction (R1–R4)
  - [ ] 1.1 Add `head_sha`, `collect_staged_changes`, and `collect_working_tree_changes` to `git.rs`
  ...
  - [ ] 1.2 Add `ChangeSource` enum and `plan_from_source` to `lib.rs`
  ...
  - [ ]* 1.3 Write property test for change source field mapping (Property 1)
  ...
  - [ ]* 1.4 Write property test for stdin path parsing (Property 2)
  ...
  - [ ] 1.5 Extend CLI `Plan` command with change source flags and mutual exclusivity validation
  ...
  - [ ]* 1.6 Write property test for change source mutual exclusivity (Property 3)
...
- [ ] 2. Checkpoint — Local Diff Planning
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 3. Traceability / Explain Engine (R5–R8)
  - [ ] 3.1 Add structured query types and `query_path` to `explain.rs`
  ...
  - [ ]* 3.2 Write property test for path query correctness (Property 4)
  ...
  - [ ] 3.3 Add `query_obligation` and `query_surface` to `explain.rs`
  ...
  - [ ]* 3.4 Write property tests for obligation and surface query correctness (Properties 5, 6)
  ...
  - [ ] 3.5 Add `trace_plan` to `explain.rs`
  ...
  - [ ]* 3.6 Write property test for trace completeness (Property 7)
  ...
  - [ ] 3.7 Extend CLI with `explain --path/--obligation/--surface` and `trace` subcommands
...
- [ ] 4. Checkpoint — Traceability / Explain
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 5. CI Matrix Emission (R9–R11)
  - [ ] 5.1 Add `emit_matrix_json`, `emit_structured_json`, `emit_nextest_filtersets` to `emit.rs`
  ...
  - [ ]* 5.2 Write property tests for new emitters (Properties 8, 9, 10, 19)
  ...
  - [ ] 5.3 Extend CLI `Emit` subcommand with `Matrix`, `Json`, `NextestFiltersets` variants
...
- [ ] 6. Checkpoint — CI Matrix Emission
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 7. Semantic Doctor Extensions (R12–R17)
  - [ ] 7.1 Add `DoctorFinding` type and extend `DoctorReport` in `doctor.rs`
  ...
  - [ ] 7.2 Implement duplicate surface ID detection in `doctor.rs`
  ...
  - [ ]* 7.3 Write property test for duplicate surface detection (Property 11)
  ...
  - [ ] 7.4 Implement uncovered obligation and unreachable rule detection in `doctor.rs`
  ...
  - [ ] 7.5 Implement unbound placeholder detection in `doctor.rs`
  ...
  - [ ]* 7.6 Write property test for unbound placeholder detection (Property 12)
  ...
  - [ ] 7.7 Implement missing tool detection in `doctor.rs`
  ...
  - [ ] 7.8 Add `--strict` flag to CLI `Doctor` command
  ...
  - [ ]* 7.9 Write property test for strict mode exit behavior (Property 13)
...
- [ ] 8. Checkpoint — Semantic Doctor
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 9. Run Resume / Failed-Only (R18–R19)
  - [ ] 9.1 Implement `execute_with_resume` in `run.rs`
  ...
  - [ ] 9.2 Implement `execute_failed_only` in `run.rs`
    - Verify `previous_receipt.plan_digest == plan.plan_digest`; return `Err` on mismatch
  ...
  - [ ]* 9.3 Write property tests for resume and failed-only (Properties 14, 15, 16)
  ...
  - [ ] 9.4 Extend CLI `Run` command with `--resume` and `--failed-only` flags
...
- [ ] 10. Checkpoint — Run Resume / Failed-Only
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 11. Plan Comparison and Budget Gates (R20–R24)
  - [ ] 11.1 Create `compare.rs` with `compare_plans` function
  ...
  - [ ]* 11.2 Write property test for plan comparison (Property 17)
  ...
  - [ ] 11.3 Add `Compare` subcommand to CLI
  ...
  - [ ] 11.4 Implement budget gate checks in CLI `Plan` command
  ...
  - [ ]* 11.5 Write property test for budget gates (Property 18)
...
- [ ] 12. Checkpoint — Plan Comparison and Budget Gates
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 13. Cross-Cutting Concerns (R25–R27)
  - [ ] 13.1 Verify deterministic output and artifact shape preservation
  ...
  - [ ] 13.2 Verify plan loading for explain and trace commands
